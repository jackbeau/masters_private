...
class CustomToolbar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<AppBarBloc>(
      create: (context) => AppBarBloc(BlocProvider.of<ScriptEditorBloc>(context).state.pdfController!),
      child: BlocBuilder<ScriptEditorBloc, ScriptEditorState>(
        builder: (context, state) {
          final appBarBloc = BlocProvider.of<AppBarBloc>(context);
          final screenWidth = MediaQuery.of(context).size.width;
          final Mode currentMode = BlocProvider.of<ScriptEditorBloc>(context).state.mode;
          final InspectorPanel selectedInspector = BlocProvider.of<ScriptEditorBloc>(context).state.selectedInspector;
          final bool isCameraActive = BlocProvider.of<ScriptEditorBloc>(context).state.isCameraVisible;
          final Tool selectedTool = BlocProvider.of<ScriptEditorBloc>(context).state.selectedTool;
                ...
                Expanded(
                  ...
                  child: Row(
                    children: [
                      ...
                      if (BlocProvider.of<ScriptEditorBloc>(context).state.mode == Mode.edit)
                        IconButton(
                          icon: IconHelper.getIcon(context, ThemeIcons.cueTool, selectedTool == Tool.newCue, width: 24),
                          onPressed: () => BlocProvider.of<ScriptEditorBloc>(context).add(ToolChanged(selectedTool == Tool.newCue ? Tool.none : Tool.newCue)),
                          tooltip: "Add cue",
                        ),
                    ],
                  ),
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          BlocProvider.of<ScriptEditorBloc>(context).add(ToggleCameraView());
                        },
