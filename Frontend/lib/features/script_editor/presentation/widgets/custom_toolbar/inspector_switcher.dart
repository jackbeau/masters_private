  ...
  Widget build(BuildContext context) {
    return SegmentedButton<InspectorPanel>(
      ...
      onSelectionChanged: (Set<InspectorPanel> i) {
        if (i.isNotEmpty) {
          BlocProvider.of<ScriptEditorBloc>(context).add(InspectorChanged(i.first));
        }
      },
      style: ButtonStyle(
        ...
        padding: MaterialStateProperty.all(const EdgeInsets.symmetric(vertical: 0, horizontal: 18)), // Modified padding for visual spacing
      ),
    );
  }
}
