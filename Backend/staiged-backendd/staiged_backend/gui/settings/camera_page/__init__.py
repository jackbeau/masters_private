...
import logging
from gui.settings.settings_manager import CameraSettingsManager
from multiprocessing import Manager
from gui.settings.camera_page.video_processing import video_loop
from gui.settings.camera_page.plot_processing import plot_loop
...
class CameraPage(tk.Frame):
    ...
    def get_video_device_info(self):
        ...
        try:
            ...
            if device:
                ...
                print(f"Selected device position: {self.video_device_position}")
    ...
    def load_settings(self):
        ...
        self.clahe_clip_limit = tk.DoubleVar(value=camera_settings.get('clahe_clip_limit', 40))
    ...
    def close(self):
        ...
        self.plot_process.join()  # This should now not hang

    def configure_video_device(self):
        print("55")
        if self.video_device_position is not None:
            self.max_res = utils.video_utils.get_max_resolution(self.video_device_position)
        else: 
            self.max_res = None
        ...
        if self.max_res is not None:
            ...
            self.video_device_str = tk.StringVar(
                value=self.video_device_name)
        else:
            self.res_list = None
    ...
    def start_processes(self):
        ...
        self.plot_process = multiprocessing.Process(
            target=plot_loop, args=(self.hist_queue, self.frame_queue, self.stop_event, self.settings)
        )
    ...
    def render(self):
        ...
        self.master.bind(
            ...
            lambda event: (
                ...
                self.master.after(
                    1000,
                    lambda: setattr(self, 'resizing', False)
                )
            )
        )
        ...
        btn_mirror_horizontal = ttk.Checkbutton(
            frm_row_3, text="Mirror Horizontal",
        ...
        btn_mirror_vertical = ttk.Checkbutton(
            frm_row_3, text="Mirror Vertical",
            style="Label.TCheckbutton",
        ...
        self.hist_label = ttk.Label(
            frm_row_5, image=None, anchor="center", justify="center", background=colours.off_black_80)
        ...
        btn_clahe = ttk.Checkbutton(
            frm_row_5_col_1, text="CLAHE Equalisation",
            style="Label.TCheckbutton",
            variable=self.clahe,
            onvalue=1,
            offvalue=0,
        ...
        lbl_exposure = text(
            frm_row_5_col_1, text="Exposure", style="PageText"
        )
        ...
        lbl_contrast = text(
            frm_row_5_col_1, text="Contrast", style="PageText"
        )
    ...
    def select_video_device(self, device_idn):
        ...
        if device_idn != self.video_device_name:
            new_device = next((i for i in self.video_devices if i["localizedName"] == device_idn), None)
            ...
            self.save_settings()
            try:
                # Set the stop event and ensure the process is terminated
                self.stop_event.set()
                self.video_process.terminate()
                self.video_process.join()  # This should now not hang
                self.plot_process.terminate()
